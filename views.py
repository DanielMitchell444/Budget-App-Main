...
def load_cert_and_key():
    ...
    # Ensure cert_path and key_path are valid files before using them
    if not os.path.exists(CERT_PATH) or not os.path.exists(KEY_PATH):
        raise ValueError("Certificate or private key path does not exist.")
...
def create_link_token(request):
    ...
    # Prepare Teller API request data
    data = {
        "application": {
            "name": "budget",  # Replace with your application name
        },
        "user": {
            "id": str(user.id),  # Use Django's default `id` as the unique identifier
        },
    }

    # Prepare headers and certificate paths
    headers = {
    ...
    if not cert_path or not key_path or not os.path.exists(cert_path) or not os.path.exists(key_path):
        return JsonResponse({"error": "Certificate or private key path is invalid."}, status=500)
    ...
    try:
        ...
        response = requests.post(
            "https://api.teller.io/v1/link",
            headers=headers,
            json=data,
            cert=cert
        )

        # Handle the Teller API response
...
def exchange_public_token(request):
    ...
    headers = {
        "Authorization": f"Bearer {settings.TELLER_API_KEY}"
    }
    ...
    if response.status_code == 200:
        ...
        return JsonResponse({"error": "Failed to exchange public token"}, status=500)
